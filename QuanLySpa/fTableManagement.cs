...
namespace QuanLySpa
{
    public partial class fTableManagement : Form
    {
        ...
        {
            List<Service> listService = ServiceDAO.Instance.GetServiceByCategoryID(id);
        ...
        private void f_UpdateService(object sender, EventArgs e)
        {
            LoadServiceListByCategoryID((cbCategory.SelectedItem as Category).ID);
            if (lsvBill.Tag != null)
                ShowBill((lsvBill.Tag as Table).ID);
        }

        private void f_DeleteService(object sender, EventArgs e)
        {
            LoadServiceListByCategoryID((cbCategory.SelectedItem as Category).ID);
            if (lsvBill.Tag != null)
                ShowBill((lsvBill.Tag as Table).ID);
        ...
        private void f_InsertService(object sender, EventArgs e)
        {
            LoadServiceListByCategoryID((cbCategory.SelectedItem as Category).ID);
            if (lsvBill.Tag != null)
                ShowBill((lsvBill.Tag as Table).ID);
        ...
        private void btnAddService_Click(object sender, EventArgs e)
        {
            ...
            if(idBill == -1)
            {
                ...
                BillInforDAO.Instance.InsertBillInfo(BillDAO.Instance.GetMaxIDBill(),serviceID,count);

            }
            else
            {
            
                BillInforDAO.Instance.InsertBillInfo(idBill, serviceID, count);               
            }
